...
function(info, value) E.db.nameplates.units[unit][auraType][info[#info]] = value NP:ConfigureAll() end)

aura.args.enable       = ACH:Toggle(L["Enable"], nil, 1)
aura.args.stackAuras   = ACH:Toggle(L["Stack Auras"], L["This will join auras together which are normally separated."], 2)
aura.args.desaturate   = ACH:Toggle(L["Desaturate Icon"], L["Set auras that are not from you to desaturated."], 3)
aura.args.keepSizeRatio = ACH:Toggle(L["Keep Size Ratio"], nil, 4)
aura.args.size         = ACH:Range(function() return E.db.nameplates.units[unit][auraType].keepSizeRatio and L["Size"] or L["Width"] end, nil, 5, { min = 6, max = 60, step = 1 })
aura.args.height       = ACH:Range(L["Height"], nil, 6, { min = 6, max = 60, step = 1 }, nil, nil, nil, nil, function() return E.db.nameplates.units[unit][auraType].keepSizeRatio end)
aura.args.numAuras     = ACH:Range(L["Per Row"], nil, 7, { min = 1, max = 20, step = 1 })
aura.args.numRows      = ACH:Range(L["Num Rows"], nil, 8, { min = 1, max = 5, step = 1 })
aura.args.spacing      = ACH:Range(L["Spacing"], nil, 9, { min = 0, max = 60, step = 1 })
aura.args.xOffset      = ACH:Range(L["X-Offset"], nil, 10, { min = -100, max = 100, step = 1 })
aura.args.yOffset      = ACH:Range(L["Y-Offset"], nil, 11, { min = -100, max = 100, step = 1 })
aura.args.anchorPoint  = ACH:Select(L["Anchor Point"], L["What point to anchor to the frame you set to attach to."], 12, C.Values.Anchors)
...
local smartHideKey1 = auraType == 'buffs' and 'BUFFS_ON_DEBUFFS'  or 'DEBUFFS_ON_BUFFS'
local smartHideKey2 = auraType == 'buffs' and 'FLUID_BUFFS_ON_DEBUFFS' or 'FLUID_DEBUFFS_ON_BUFFS'
aura.args.attachTo = ACH:Select(L["Attach To"], L["What to attach the anchor frame to."], 13, attachValues, nil, nil, nil, nil,
function() local pos = E.db.nameplates.units[unit].smartAuraPosition return pos == smartHideKey1 or pos == smartHideKey2 end)

aura.args.growthX = ACH:Select(L["Growth X-Direction"], nil, 14, { LEFT = L["Left"], RIGHT = L["Right"] }, nil, nil, nil, nil,
function() local p = E.db.nameplates.units[unit][auraType].anchorPoint return p == 'LEFT' or p == 'RIGHT' end)
aura.args.growthY = ACH:Select(L["Growth Y-Direction"], nil, 15, { UP = L["Up"], DOWN = L["Down"] }, nil, nil, nil, nil,
function() local p = E.db.nameplates.units[unit][auraType].anchorPoint return p == 'TOP' or p == 'BOTTOM' end)
aura.args.sortMethod   = ACH:Select(L["Sort By"],        L["Method to sort by."],                    16, { TIME_REMAINING = L["Time Remaining"], DURATION = L["Duration"], NAME = L["Name"], INDEX = L["Index"], PLAYER = L["Player"] })
aura.args.sortDirection = ACH:Select(L["Sort Direction"], L["Ascending or Descending order."],        17, { ASCENDING = L["Ascending"], DESCENDING = L["Descending"] })
...
aura.args.stacks.args.countFontSize    = ACH:Range(L["Font Size"],              nil, 2, { min = 4, max = 60, step = 1 })
...
aura.args.stacks.args.countXOffset     = ACH:Range(L["X-Offset"],              nil, 5, { min = -100, max = 100, step = 1 })
aura.args.stacks.args.countYOffset     = ACH:Range(L["Y-Offset"],              nil, 6, { min = -100, max = 100, step = 1 })

aura.args.duration = ACH:Group(L["Duration"], nil, 25)
aura.args.duration.inline = true
aura.args.duration.args.cooldownShortcut = ACH:Execute(L["Cooldowns"], nil, 1, function() ACD:SelectGroup('ElvUI', 'cooldown', 'nameplates') end)
...
aura.args.filtersGroup.args.minDuration = ACH:Range(L["Minimum Duration"], L["Don't display auras that are shorter than this duration (in seconds). Set to zero to disable."], 1, { min = 0, max = 10800, step = 1 })
aura.args.filtersGroup.args.maxDuration = ACH:Range(L["Maximum Duration"], L["Don't display auras that are longer than this duration (in seconds). Set to zero to disable."],  2, { min = 0, max = 10800, step = 1 })
aura.args.filtersGroup.args.jumpToFilter = ACH:Execute(L["Filters Page"], L["Shortcut to global filters."], 3, function() ACD:SelectGroup('ElvUI', 'filters') end)

aura.args.filtersGroup.args.specialFilters = ACH:Select(L["Add Special Filter"],
L["These filters don't use a list of spells like the regular filters. Instead they use the WoW API and some code logic to determine if an aura should be allowed or blocked."], 4,
function() wipe(filters) local list = E.global.unitframe.specialFilters if not (list and next(list)) then return filters end for filter in pairs(list) do filters[filter] = L[filter] end return filters end,
nil, nil, nil,
function(_, value) C.SetFilterPriority(E.db.nameplates.units, unit, auraType, value) NP:ConfigureAll() end)
...
L["These filters use a list of spells to determine if an aura should be allowed or blocked. The content of these filters can be modified in the Filters section of the config."], 5,
function() wipe(filters) local list = E.global.unitframe.aurafilters if not (list and next(list)) then return filters end for filter in pairs(list) do filters[filter] = L[filter] end return filters end,
nil, nil, nil,
function(_, value) C.SetFilterPriority(E.db.nameplates.units, unit, auraType, value) NP:ConfigureAll() end)

aura.args.filtersGroup.args.resetPriority = ACH:Execute(L["Reset Priority"], L["Reset filter priority to the default state."], 7,
function() E.db.nameplates.units[unit][auraType].priority = P.nameplates.units[unit][auraType].priority NP:ConfigureAll() end)

aura.args.filtersGroup.args.filterPriority = ACH:MultiSelect(L["Filter Priority"], nil, 8,
function() local str = E.db.nameplates.units[unit][auraType].priority if str == '' then return {} end return {strsplit(',', str)} end,
nil, nil,
function(_, value) local str = E.db.nameplates.units[unit][auraType].priority if str == '' then return end local tbl = {strsplit(',', str)} return tbl[value] end,
...
aura.args.filtersGroup.args.filterPriority.dragOnMouseDown = function(info) carryFilterFrom, carryFilterTo = info.obj.value, nil end
aura.args.filtersGroup.args.filterPriority.dragOnMouseUp   = function() C.SetFilterPriority(E.db.nameplates.units, unit, auraType, carryFilterTo, nil, carryFilterFrom) carryFilterFrom, carryFilterTo = nil, nil end
aura.args.filtersGroup.args.filterPriority.dragOnClick     = function() C.SetFilterPriority(E.db.nameplates.units, unit, auraType, carryFilterFrom, true) end
aura.args.filtersGroup.args.filterPriority.stateSwitchGetText  = C.StateSwitchGetText
aura.args.filtersGroup.args.filterPriority.stateSwitchOnClick  = function() C.SetFilterPriority(E.db.nameplates.units, unit, auraType, carryFilterFrom, nil, nil, true) end
...
function(info, value) E.db.nameplates.units[unit][info[#info]] = value NP:ConfigureAll() end,
function() return not NP.Initialized end)

group.args.enable          = ACH:Toggle(L["Enable"], nil, -10)
group.args.showTestFrame   = ACH:Execute(L["Show/Hide Test Frame"], nil, -9, function() NP:TogleTestFrame(unit) end)
...
-- General
group.args.general = ACH:Group(L["General"], nil, 1, nil,
function(info) return E.db.nameplates.units[unit][info[#info]] end,
function(info, value) E.db.nameplates.units[unit][info[#info]] = value NP:UpdateCVars() NP:ConfigureAll() end)
...
-- Health
group.args.healthGroup = ACH:Group(L["Health"], nil, 10, nil,
function(info) return E.db.nameplates.units[unit].health[info[#info]] end,
function(info, value) E.db.nameplates.units[unit].health[info[#info]] = value NP:ConfigureAll() end)
...
function(info, value) E.db.nameplates.units[unit].health.text[info[#info]] = value NP:ConfigureAll() end)
...
function(info, value) E.db.nameplates.units[unit].power[info[#info]] = value NP:ConfigureAll() end)
...
group.args.powerGroup.args.classColor   = ACH:Toggle(L["Use Class Color"], nil, 7)
...
function(info, value) E.db.nameplates.units[unit].power.text[info[#info]] = value NP:ConfigureAll() end)
...
-- Cast Bar
group.args.castGroup = ACH:Group(L["Cast Bar"], nil, 20, nil,
function(info) return E.db.nameplates.units[unit].castbar[info[#info]] end,
function(info, value) E.db.nameplates.units[unit].castbar[info[#info]] = value NP:ConfigureAll() end)
group.args.castGroup.args.enable                    = ACH:Toggle(L["Enable"], nil, 1)
group.args.castGroup.args.sourceInterrupt           = ACH:Toggle(L["Display Interrupt Source"], L["Display the unit name who interrupted a spell on the castbar."], 2)
...
group.args.castGroup.args.iconGroup.args.iconPosition = ACH:Select(L["Position"], nil, 2, { LEFT = L["Left"], RIGHT = L["Right"] })
...
-- Buffs / Debuffs
group.args.buffsGroup   = BuildAuraGroup(unit, 'buffs',   L["Buffs"],   25)
group.args.debuffsGroup = BuildAuraGroup(unit, 'debuffs', L["Debuffs"], 30)

-- Portrait
group.args.portraitGroup = ACH:Group(L["Portrait"], nil, 40, nil,
function(info) return E.db.nameplates.units[unit].portrait[info[#info]] end,
function(info, value) E.db.nameplates.units[unit].portrait[info[#info]] = value NP:ConfigureAll() end)
...
function(info, value) E.db.nameplates.units[unit].level[info[#info]] = value NP:ConfigureAll() end)
group.args.levelGroup.args.enable     = ACH:Toggle(L["Enable"], nil, 1)
group.args.levelGroup.args.textFormat = ACH:Input(L["Text Format"], nil, 2, nil, 'full')
...
function(info, value) E.db.nameplates.units[unit].name[info[#info]] = value NP:ConfigureAll() end)
group.args.nameGroup.args.enable     = ACH:Toggle(L["Enable"], nil, 1)
group.args.nameGroup.args.textFormat = ACH:Input(L["Text Format"], nil, 2, nil, 'full')
...
group.args.raidTargetIndicator = ACH:Group(L["Target Marker Icon"], nil, 65, nil,
function(info) return E.db.nameplates.units[unit].raidTargetIndicator[info[#info]] end,
function(info, value) E.db.nameplates.units[unit].raidTargetIndicator[info[#info]] = value NP:ConfigureAll() end)
...
group.args.pvpindicator = ACH:Group(L["PvP Indicator"], L["Horde / Alliance / Honor Info"], 60, nil,
function(info) return E.db.nameplates.units[unit].pvpindicator[info[#info]] end,
function(info, value) E.db.nameplates.units[unit].pvpindicator[info[#info]] = value NP:ConfigureAll() end)
...
function(info, value) E.db.nameplates.units[unit].eliteIcon[info[#info]] = value NP:ConfigureAll() end)
...
function(info, value) E.db.nameplates[info[#info]] = value NP:ConfigureAll() end)
local NamePlates = E.Options.args.nameplates.args

NamePlates.intro     = ACH:Description(L["NAMEPLATE_DESC"], 0)
NamePlates.enable    = ACH:Toggle(L["Enable"], nil, 1, nil, nil, nil,
function(info) return E.private.nameplates[info[#info]] end,
function(info, value) E.private.nameplates[info[#info]] = value E.ShowPopup = true end)
...
function(info, value) E.db.nameplates[info[#info]] = value NP:UpdateCVars() NP:ConfigureAll() end,
function() return not NP.Initialized end)
...
NamePlates.generalGroup.args.showEnemyCombat    = ACH:Select(L["Enemy Combat Toggle"],   L["Control enemy nameplates toggling on or off when in combat."],   2, { DISABLED = L["Disable"], TOGGLE_ON = L["Toggle On While In Combat"], TOGGLE_OFF = L["Toggle Off While In Combat"] }, nil, nil, nil,
function(info, value) E.db.nameplates[info[#info]] = value NP:PLAYER_REGEN_ENABLED() end)
NamePlates.generalGroup.args.showFriendlyCombat = ACH:Select(L["Friendly Combat Toggle"], L["Control friendly nameplates toggling on or off when in combat."], 3, { DISABLED = L["Disable"], TOGGLE_ON = L["Toggle On While In Combat"], TOGGLE_OFF = L["Toggle Off While In Combat"] }, nil, nil, nil,
function(info, value) E.db.nameplates[info[#info]] = value NP:PLAYER_REGEN_ENABLED() end)
NamePlates.generalGroup.args.smoothbars         = ACH:Toggle(L["Smooth Bars"],      L["Bars will transition smoothly."],                                     4)
NamePlates.generalGroup.args.clampToScreen      = ACH:Toggle(L["Clamp Nameplates"], L["Clamp nameplates to the top of the screen when outside of view."],   5)
...
NamePlates.generalGroup.args.plateVisibility.args.showAll = ACH:Toggle(L["UNIT_NAMEPLATES_AUTOMODE"], L["This option controls the Blizzard setting for whether or not the Nameplates should be shown."], 0, nil, nil, nil,
function(info) return E.db.nameplates.visibility[info[#info]] end,
function(info, value) E.db.nameplates.visibility[info[#info]] = value NP:UpdateCVars() NP:ConfigureAll() end)
...
function(_, key, value) E.db.nameplates.visibility.enemy[key] = value NP:UpdateCVars() NP:ConfigureAll() end,
...
function(_, key, value) E.db.nameplates.visibility.friendly[key] = value NP:UpdateCVars() NP:ConfigureAll() end,
function() return not E.db.nameplates.visibility.showAll end)
...
NamePlates.generalGroup.args.clickThrough.args.friendly = ACH:Toggle(L["Friendly"], nil, 1, nil, nil, nil, nil,
function(info, value) E.db.nameplates.clickThrough[info[#info]] = value NP:ConfigureAll() end)
NamePlates.generalGroup.args.clickThrough.args.enemy    = ACH:Toggle(L["Enemy"],    nil, 2, nil, nil, nil, nil,
function(info, value) E.db.nameplates.clickThrough[info[#info]] = value NP:ConfigureAll() end)

NamePlates.generalGroup.args.clickableRange = ACH:Group(L["Clickable Size"], nil, 70, nil,
function(info) return E.db.nameplates.plateSize[info[#info]] end,
function(info, value) E.db.nameplates.plateSize[info[#info]] = value NP:ConfigureAll() end)
...
function(info, value) E.db.nameplates.cutaway.health[info[#info]] = value NP:ConfigureAll() end)
...
function(info, value) E.db.nameplates.cutaway.power[info[#info]] = value NP:ConfigureAll() end)
...
function(info, value) E.db.nameplates.threat[info[#info]] = value NP:ConfigureAll() end)
...
NamePlates.generalGroup.args.targetIndicator = ACH:Group(L["Target Indicator"], nil, 85, nil,
function(info) return E.db.nameplates.targetIndicator[info[#info]] end,
function(info, value) E.db.nameplates.targetIndicator[info[#info]] = value NP:ConfigureAll() end)
...
NamePlates.colorsGroup = ACH:Group(L["Colors"], nil, 15, nil, nil, nil, function() return not NP.Initialized end)

NamePlates.colorsGroup.args.general = ACH:Group(L["General"], nil, 1, nil,
function(info) local t, d = E.db.nameplates.colors[info[#info]], P.nameplates.colors[info[#info]] return t.r, t.g, t.b, t.a, d.r, d.g, d.b, d.a end,
function(info, r, g, b, a) local t = E.db.nameplates.colors[info[#info]] t.r, t.g, t.b, t.a = r, g, b, a NP:ConfigureAll() end)
...
function(info) local t, d = E.db.nameplates.colors.threat[info[#info]], P.nameplates.colors.threat[info[#info]] return t.r, t.g, t.b, t.a, d.r, d.g, d.b, d.a end,
function(info, r, g, b, a) local t = E.db.nameplates.colors.threat[info[#info]] t.r, t.g, t.b, t.a = r, g, b, a NP:ConfigureAll() end,
...
function(info) local t, d = E.db.nameplates.colors[info[#info]], P.nameplates.colors[info[#info]] return t.r, t.g, t.b, t.a, d.r, d.g, d.b end,
function(info, r, g, b) local t = E.db.nameplates.colors[info[#info]] t.r, t.g, t.b = r, g, b NP:ConfigureAll() end)
...
NamePlates.colorsGroup.args.castGroup.args.castbarDesaturate    = ACH:Toggle(L["Desaturated Icon"], L["Show the castbar icon desaturated if a spell is not interruptible."], 4, nil, nil, nil,
function(info) return E.db.nameplates.colors[info[#info]] end,
function(info, value) E.db.nameplates.colors[info[#info]] = value NP:ConfigureAll() end)

NamePlates.colorsGroup.args.reactions = ACH:Group(L["Reaction Colors"], nil, 5, nil,
function(info) local t, d = E.db.nameplates.colors.reactions[info[#info]], P.nameplates.colors.reactions[info[#info]] return t.r, t.g, t.b, t.a, d.r, d.g, d.b end,
function(info, r, g, b) local t = E.db.nameplates.colors.reactions[info[#info]] t.r, t.g, t.b = r, g, b NP:ConfigureAll() end)
...
NamePlates.unitsGroup = ACH:Group(L["Units"], nil, 20, 'tree', nil, nil, function() return not NP.Initialized end)
local Units = NamePlates.unitsGroup.args

Units.FRIENDLY_PLAYER = GetUnitSettings('FRIENDLY_PLAYER', L["FRIENDLY_PLAYER"])
